    ...
    if (userType === 'technical') {
      await dbRun(
        `INSERT INTO technical_profiles (user_id, skills, previous_projects, github_url, portfolio_url)
         VALUES (?, ?, ?, ?, ?)`,
        userId, skills || '', previousProjects || '', githubUrl || '', portfolioUrl || ''
      );
    } else {
      await dbRun(
        `INSERT INTO non_technical_profiles (user_id, idea_title, idea_description, timeline, equity_offered, budget)
         VALUES (?, ?, ?, ?, ?, ?)`,
        userId, ideaTitle || '', ideaDescription || '', timeline || '', equityOffered || '', budget || ''
      );
    }
...
app.get('/api/user', requireAuth, async (req, res) => {
  try {
    ...
    if (user.user_type === 'technical') {
      profile = await dbGet('SELECT * FROM technical_profiles WHERE user_id = ?', user.id);
    } else {
      profile = await dbGet('SELECT * FROM non_technical_profiles WHERE user_id = ?', user.id);
    }
...
app.get('/api/potential-matches', requireAuth, async (req, res) => {
  try {
    const currentUser = await dbGet('SELECT user_type FROM users WHERE id = ?', req.session.userId);
    ...
    const potentialMatches = await dbAll(
      `SELECT u.id, u.name, u.user_type, u.country, u.university, u.bio
       ...
       ORDER BY RANDOM()
       LIMIT 10`,
      targetType, req.session.userId, req.session.userId
    );

    // Get profiles for each user
    const matchesWithProfiles = await Promise.all(potentialMatches.map(async (user) => {
      let profile = null;
      if (user.user_type === 'technical') {
        profile = await dbGet('SELECT * FROM technical_profiles WHERE user_id = ?', user.id);
      } else {
  ...
  try {
    ...
    await dbRun(
      `INSERT OR IGNORE INTO swipes (swiper_id, swiped_id, direction)
       VALUES (?, ?, ?)`,
      req.session.userId, targetUserId, direction
    );

    // Check for match if it's a right swipe
    let isMatch = false;
    if (direction === 'right') {
      const mutualSwipe = await dbGet(
        `SELECT * FROM swipes
  ...
  try {
    ...
    const matchesWithDetails = await Promise.all(matches.map(async (match) => {
      ...
      if (match.user_type === 'technical') {
        profile = await dbGet('SELECT * FROM technical_profiles WHERE user_id = ?', match.id);
      } else {
        profile = await dbGet('SELECT * FROM non_technical_profiles WHERE user_id = ?', match.id);
      }
  ...
  try {
    ...
    const result = await dbRun(
      `INSERT INTO messages (match_id, sender_id, content)
       VALUES (?, ?, ?)`,
      matchId, req.session.userId, content
    );
